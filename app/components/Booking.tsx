    ...
    <section id="booking">
      <div className="mx-8 my-16 p-6 lg:p-12 lg:mx-28 bg-white bg-book-bg rounded shadow-white-box space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-end">
          {bookingInputs.select.map((data) => (
            <div key={data.id} className="flex flex-col gap-4">
              <label htmlFor={data.htmlId} className="flex items-center gap-2">
                <span className="text-custom-purple text-xl">
                  <data.label.icon />
                </span>
                <span className="font-semibold">{data.label.text}</span>
                <span className="text-custom-purple font-bold">*</span>
              </label>
              <select
                id={data.htmlId}
          ...
          <div>
            <button
              className="bg-custom-purple w-full shadow-purple-bottom hover:shadow-purple-bottom-hov transition-all duration-300 ease-linear text-white p-2 font-bold rounded"
              onClick={() => {
                setBookingFields({
                  ...bookingFields,
                  red:
